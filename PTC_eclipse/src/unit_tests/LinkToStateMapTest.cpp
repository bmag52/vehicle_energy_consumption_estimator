...
void linkToStateMap_ut() {
	...
	for(int i = 0; i < linksSize; i++) {
		int random = rand() % 5; //is this correct for values?
		links[i] = Link(i, random);
	}
...
//	LinkToStateMap map;
//	map.incrementTransition((links + 0), goal1, (links + 1));
//
//	// Test 2: increment transition twice
//	LinkToStateMap map2;
//	for(int i = 0; i < 2; i++) {
//		map2.incrementTransition(links + 1, goal2, links + 3);
//	}
//
//	// Test 3: increment two different transitions
//	LinkToStateMap map3;
//	map3.incrementTransition(links + 3, goal1, links + 0);
//	map3.incrementTransition(links[4], goal2, links[1]);
//
